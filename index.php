...
if (isset($_GET['action']) && $_GET['action'] == 'history') {
  ...
  <div class="bnt btn-group"><div class="btn btn-info" data-toggle="collapse" data-target="#batchItemDetail" aria-expanded="false" aria-controls="batchItemDetail">' . __('Options') . '</div>';
    ...
    $form->addSelectList('publisherID', __('Publisher'), $publ_options, $rec_d['publisher_id'] ?? '', 'class="select2" data-src="' . $_SERVER['PHP_SELF'] . '" data-src-table="mst_publisher" data-src-cols="publisher_id:publisher_name"');
    // biblio publish year
    ...
    $form->addSelectList('placeID', __('Publishing Place'), $plc_options, $rec_d['publish_place_id'] ?? '', 'class="select2" data-src="' . $_SERVER['PHP_SELF'] . '" data-src-table="mst_place" data-src-cols="place_id:place_name"');
    // biblio collation
    ...
    $form->addSelectList('class', __('Classification'), $cls_options, $rec_d['classification'] ?? '', 'class="select2" data-src="' . $_SERVER['PHP_SELF'] . '" data-src-table="mst_topic" data-src-cols="classification:classification:topic"');
    // biblio call_number
